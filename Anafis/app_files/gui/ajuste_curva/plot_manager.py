...
if TYPE_CHECKING:
    ...
    # Extend matplotlib's Line2D to include our custom attribute
    class ExtendedLine2D(Line2D):
...
class PlotManager:
    ...
    def plot_fit_results(self, 
        ...
        if x_scale == 'log':
            ...
            if x_min_val <= 0:
                ...
                if positive_x.size > 0:
                    ...
                    logging.warning("Log scale requested for non-positive data. Using linear scale for fit curve.")
        ...
        try:            # Type annotation for beta array from ODR result
            beta_params = cast(BetaArray, result.beta)  # type: ignore[attr-defined]
            ...
            if title:
                ...
                eq_title = equation
                beta_values = cast(BetaArray, result.beta)  # type: ignore[attr-defined]
                for i, p_sym in enumerate(parameters):
                    eq_title = eq_title.replace(str(p_sym), f"{str(p_sym)}={beta_values[i]:.4f}")
            ...
            # Ensure tight layout
            self.fig.tight_layout() # type: ignore[reportUnknownMemberType]            self.canvas.draw() # type: ignore[reportUnknownMemberType]
    ...
    def update_graph_appearance(self, 
        ...
        if x_label:
            self.ax.set_xlabel(x_label) # type: ignore[reportUnknownMemberType]
            self.ax_res.set_xlabel(x_label) # type: ignore[reportUnknownMemberType]
        if y_label:            self.ax.set_ylabel(y_label) # type: ignore[reportUnknownMemberType]
        self.fig.tight_layout() # type: ignore[reportUnknownMemberType]
        self.canvas.draw() # type: ignore[reportUnknownMemberType]
    ...
    def update_legend(self) -> None:
        ...
        handles, labels = self.ax.get_legend_handles_labels() # type: ignore[reportUnknownMemberType]
        
        # If there are no handles or labels, remove the legend completely
        if not handles and not labels:
            legend = self.ax.get_legend() # type: ignore[reportUnknownMemberType]
            if legend:
                legend.remove() # type: ignore[reportUnknownMemberType]
        else:
            # Create/update legend with current handles and labels
            self.ax.legend(handles, labels) # type: ignore[reportUnknownMemberType]
    ...
    def plot_custom_functions(self, custom_functions: List['CustomFunction']) -> None:
        ...
        """
        import logging
        logging.debug(f"plot_custom_functions called with {len(custom_functions)} functions")
        ...
        for line in self.ax.get_lines(): # type: ignore[reportUnknownMemberType]
            if hasattr(line, 'is_custom_function') and getattr(line, 'is_custom_function', False):
                lines_to_remove.append(line)
        
        logging.debug(f"Removing {len(lines_to_remove)} existing custom function lines")
        for line in lines_to_remove:
            line.remove() # type: ignore[reportUnknownMemberType]
        
        # If no functions to plot, just clear and return
        if not custom_functions:
            logging.debug("No custom functions to plot - clearing and updating")
            self.update_legend()
            self.canvas.draw() # type: ignore[reportUnknownMemberType] - Force immediate redraw instead of draw_idle
            return
        
        # Get current x range for plotting custom functions
        if self.ax.get_xlim(): # type: ignore[reportUnknownMemberType]
            x_min, x_max = self.ax.get_xlim() # type: ignore[reportUnknownMemberType]
        ...
        for func in custom_functions:
            try:
                ...
                if func.x_min is not None and func.x_max is not None:
                    ...
                    if self.ax.get_xlim(): # type: ignore[reportUnknownMemberType]
                        _, axis_max = self.ax.get_xlim() # type: ignore[reportUnknownMemberType]
                    ...
                    if self.ax.get_xlim(): # type: ignore[reportUnknownMemberType]
                        axis_min, _ = self.ax.get_xlim() # type: ignore[reportUnknownMemberType]
                ...
                preprocessed_expression = preprocess_implicit_multiplication(func.func_text)
                  # Parse the expression with the same comprehensive function dictionary
                local_dict = SUPPORTED_SYMPY_OBJECTS
                expr = sp.sympify(preprocessed_expression, locals=local_dict)  # type: ignore
                ...
                if func.x_min is not None or func.x_max is not None:
                  ...
                  # Plot the function
                line, = self.ax.plot(x, y, color=func.color, linestyle='--', label=label)  # type: ignore[reportUnknownMemberType]
                # Add custom attribute to identify this as a custom function
                setattr(line, 'is_custom_function', True)  # type: ignore[attr-defined]
        ...
        self.update_legend()
        self.canvas.draw_idle()  # type: ignore[reportUnknownMemberType]
