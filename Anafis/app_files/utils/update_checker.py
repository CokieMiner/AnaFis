...
class UpdateChecker:
    ...
    def __init__(self, repo: str = "CokieMiner/AnaFis"):
        ...
        self.fallback_url = "https://raw.githubusercontent.com/CokieMiner/AnaFis/main/version.json"
    ...
    def _check_fallback_version(self) -> Dict[str, Any]:
        ...
        formatted_data = {
            ...
            "html_url": data.get("download_url", f"https://github.com/{self.repo}/releases"),
            "body": data.get("release_notes", "Check the repository for release notes.")
        }
    ...
    def show_update_dialog(self) -> None:
        ...
        try:
            from tkinter import messagebox  # type: ignore[misc]
            ...
            title = TRANSLATIONS[language].get('update_available', 'Update Available')
            message = f"{TRANSLATIONS[language].get('new_version_available', 'A new version is available')}: v{self.latest_version}\n\n"
            
            if self.release_notes:
                # Truncate release notes if too long
                notes = self.release_notes[:300] + "..." if len(self.release_notes) > 300 else self.release_notes
                message += f"{TRANSLATIONS[language].get('release_notes', 'Release Notes')}:\n{notes}\n\n"
            
            message += TRANSLATIONS[language].get('visit_download_page', 'Would you like to visit the download page?')
            
            result = messagebox.askyesno(title, message)  # type: ignore[misc]
