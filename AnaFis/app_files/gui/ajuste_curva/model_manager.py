...
from typing import (
    ...
    Optional,
    cast,
)  # Added Optional, Any, cast, removed Union
...
class ModelManager:
    ...
    def __init__(self, language: str = "pt") -> None:
        ...
        try:
            preset_models_json = get_string(
                "ajuste_curva",
                "models_presets",
                self.language,
                fallback='{"Linear: a*x + b": "a*x + b"}',
            )
            preset_models_data = (
                json.loads(preset_models_json)
                if isinstance(preset_models_json, str)
                else preset_models_json
            )
            if isinstance(preset_models_data, dict):
                self.preset_models = preset_models_data
            else:
                logging.warning(
                    f"Warning: preset_models is not a dict, got {type(preset_models_data)}: {preset_models_data}"
                )
    ...
    def create_model(
        self, equation: str, parameters: List[sp.Symbol]
    ) -> Tuple[ModelCallable, List[ModelCallable]]:
        ...
        expr: sp.Expr = cast(sp.Expr, sp.sympify(preprocessed_equation, locals=SUPPORTED_SYMPY_OBJECTS))
        derivadas_expr: List[sp.Expr] = [cast(sp.Expr, sp.diff(expr, p)) for p in parameters]
        ...
        modelo_numerico: ModelCallable = sp.lambdify((parameters, x_sym), expr, "numpy")
        derivadas_numericas: List[ModelCallable] = [sp.lambdify((parameters, x_sym), d, "numpy") for d in derivadas_expr]
        # Cache the result
    ...
    def extract_parameters(self, equation: str) -> List[sp.Symbol]:
        ...
        x_sym: sp.Symbol = sp.Symbol("x")
        # Use comprehensive function dictionary for parsing
        expr: sp.Expr = cast(sp.Expr, sp.sympify(equation_rhs, locals=SUPPORTED_SYMPY_OBJECTS))
        ...
        parameter_symbols: List[sp.Symbol] = sorted(
            list(free_symbols - {x_sym}), key=lambda s: s.name
        )
    ...
    def perform_odr_fit(
        ...
        y_pred: NDArray[np.float64] = model_func(
            resultado.beta, x
        )
    ...
    def perform_least_squares_fit(
        ...
        return perform_least_squares_fit(
            x, y, sigma_y, model_func, initial_params, max_iter
        )
    ...
    def perform_weighted_least_squares_fit(
        ...
        return perform_weighted_least_squares_fit(
            x, y, weights, model_func, initial_params, max_iter
        )
    ...
    def perform_bootstrap_fit(
        ...
        return perform_bootstrap_fit(
            x, y, sigma_y, model_func, initial_params, max_iter, n_bootstrap
        )
    ...
    def perform_bayesian_fit(
        ...
        return perform_bayesian_fit(
            x, y, sigma_y, model_func, initial_params, max_iter, n_samples
        )

    def get_help_models_content(self) -> str:
        return get_help(
            "ajuste_curva",
            "help_models_content",
            self.language,
            fallback="Ajuda sobre modelos não disponível.",
        )

    def show_help_window(self, parent):
        ...
        # Insert help text
        help_text = self.get_help_models_content()
    ...
    def show_model_help(self, parent) -> None:
        """Show a dialog with model help content in a scrollable window."""
        # Use the scrollable help window instead of messagebox
        self.show_help_window(parent)
