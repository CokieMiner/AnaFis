...
class ParameterEstimatesManager:
    ...
    def create_estimates_frame(self, parent_frame: tk.Widget) -> ttk.LabelFrame:
        ...
        self.estimates_frame = ttk.LabelFrame(
            parent_frame,
            text=get_string(
                "ajuste_curva",
                "initial_estimates",
    ...
    def setup_ui(self, tab: tk.Widget) -> None:
        ...
        if self.param_entries:
            for i, (param_name, entry) in enumerate(self.param_entries.items()):
                ...
                def update_main_entry(
                    event: tk.Event,
                    param: str = param_name,
                    popup_widget: ttk.Entry = popup_entry,
                ) -> None:  # Changed tk.Event to tk.Event[Any]
    ...
    def update_estimates_frame(
        self, equation: str
    ) -> None:  # Added type hint for equation and return
        ...
        try:
            ...
            for i, param_sym in enumerate(self.parameters):
                param_name: str = str(param_sym)
                ttk.Label(self.estimates_frame, text=f"{param_name}:").grid(
            ...
            messagebox.showerror(
                ...
                get_string(
                    ...
                    "invalid_formula",
                    self.language,
                    fallback="Fórmula inválida: {error}",
                ).format(error=str(e)),
            )

    def extract_parameters(
        self, equation: str
    ) -> List[sp.Symbol]:  # Added type hint for equation and return
        ...
        try:
            ...
            expr: sp.Expr = cast(sp.Expr, sp.sympify(equation))
            # Find all symbols in the expression
            symbols: List[sp.Symbol] = list(cast(set[sp.Symbol], expr.free_symbols))
            # Filter out 'x' which is the independent variable
            parameters_out: List[sp.Symbol] = [
                sym for sym in symbols if sym.name != "x"
            ]  # Renamed parameters to parameters_out
    ...
    def get_initial_estimates(self) -> List[float]:  # Added type hint for return
        ...
        for param_sym in self.parameters:  # Renamed param to param_sym for clarity
            param_name: str = str(
                param_sym
            )  # Explicitly convert symbol to string for key
    ...
    def switch_language(self, language: str) -> None:
        """Switch language for parameter estimates UI elements."""
        self.language = language
        try:
            if hasattr(self, "estimates_frame") and self.estimates_frame is not None:
                # Update the LabelFrame title
                try:
                    self.estimates_frame.config(
                        text=get_string(
                            "ajuste_curva", "initial_estimates", self.language
                        )
                    )
                except Exception:
                    pass
                # Update labels inside the estimates frame
                for child in self.estimates_frame.winfo_children():
                    if isinstance(child, ttk.Label):
                        # For simplicity, recreate label texts that match parameter names
                        txt = child.cget("text")
                        # If the label is a parameter name followed by ':', keep it unchanged
                        # Otherwise try to update known static labels
                        if txt.strip().endswith(":"):
                            continue
                        # Update 'Initial Values' label if present
                        if txt in [get_string("ajuste_curva", "initial_values", "pt"), get_string("ajuste_curva", "initial_values", "en")]:
                            child.config(text=get_string("ajuste_curva", "initial_values", self.language))
        except Exception:
            pass
