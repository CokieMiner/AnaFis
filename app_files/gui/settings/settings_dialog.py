...
class SettingsDialog:
    ...
    def setup_general_tab(self):
        ...
        frame = ttk.LabelFrame(self.general_tab, text=TRANSLATIONS[self.language]['general_settings'])
        ...
        language_combo.bind("<<ComboboxSelected>>", lambda e: self.mark_setting_changed('language', self.language_var.get().split(' - ')[0]))
          # Auto-save
        ...
        autosave_check.configure(command=lambda: self.mark_setting_changed('auto_save', self.autosave_var.get()))
          # Auto-backup
        self.autobackup_var = tk.BooleanVar(value=user_preferences.get_preference('auto_backup', True))
        ...
        autobackup_check.configure(command=lambda: self.mark_setting_changed('auto_backup', self.autobackup_var.get()))
          # Backup interval
        ttk.Label(frame, text=TRANSLATIONS[self.language]['backup_interval']).grid(
            row=3, column=0, sticky=tk.W, padx=10, pady=5)
        
        self.backup_interval_var = tk.StringVar(value=str(user_preferences.get_preference('backup_interval_minutes', 30)))
        ...
        backup_interval_entry.bind("<FocusOut>", lambda e: self.validate_and_mark('backup_interval_minutes', self.backup_interval_var.get(), int))
          # Check for updates
        self.check_updates_var = tk.BooleanVar(value=user_preferences.get_preference('check_updates', True))
        ...
        check_updates_check.configure(command=lambda: self.mark_setting_changed('check_updates', self.check_updates_var.get()))
    def setup_interface_tab(self):
        """Set up the interface settings tab"""
        frame = ttk.LabelFrame(self.interface_tab, text=TRANSLATIONS[self.language]['interface_settings'])
        frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        ...
        theme_combo.bind("<<ComboboxSelected>>", lambda e: self.mark_setting_changed('theme', self.theme_var.get()))
          # Font size
        ttk.Label(frame, text=TRANSLATIONS[self.language]['font_size']).grid(
            row=1, column=0, sticky=tk.W, padx=10, pady=5)
        
        self.font_size_var = tk.StringVar(value=str(user_preferences.get_preference('font_size', 12)))
        ...
        font_size_entry.bind("<FocusOut>", lambda e: self.validate_and_mark('font_size', self.font_size_var.get(), int))
          # Show tooltips
        self.tooltips_var = tk.BooleanVar(value=user_preferences.get_preference('show_tooltips', True))
        ...
        tooltips_check.configure(command=lambda: self.mark_setting_changed('show_tooltips', self.tooltips_var.get()))
          # Remember window size
        self.remember_size_var = tk.BooleanVar(value=user_preferences.get_preference('remember_window_size', True))
        ...
        remember_size_check.configure(command=lambda: self.mark_setting_changed('remember_window_size', self.remember_size_var.get()))
          # Show welcome screen
        self.welcome_screen_var = tk.BooleanVar(value=user_preferences.get_preference('show_welcome_screen', True))
        ...
        welcome_screen_check.configure(command=lambda: self.mark_setting_changed('show_welcome_screen', self.welcome_screen_var.get()))
    def setup_data_tab(self):
        """Set up the data settings tab"""
        frame = ttk.LabelFrame(self.data_tab, text=TRANSLATIONS[self.language]['data_settings'])
        frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        ...
        self.decimal_places_var = tk.StringVar(value=str(user_preferences.get_preference('decimal_places', 4)))
        decimal_places_entry = ttk.Entry(frame, textvariable=self.decimal_places_var, width=5)
        decimal_places_entry.grid(row=0, column=1, sticky=tk.W, padx=10, pady=5)
        decimal_places_entry.bind("<FocusOut>", lambda e: self.validate_and_mark('decimal_places', self.decimal_places_var.get(), int))
          # Advanced mode
        self.advanced_mode_var = tk.BooleanVar(value=user_preferences.get_preference('advanced_mode', False))
        ...
        advanced_mode_check.configure(command=lambda: self.mark_setting_changed('advanced_mode', self.advanced_mode_var.get()))
    def setup_export_tab(self):
        """Set up the export settings tab"""
        frame = ttk.LabelFrame(self.export_tab, text=TRANSLATIONS[self.language]['export_settings'])
        frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        ...
        self.graph_dpi_var = tk.StringVar(value=str(user_preferences.get_preference('graph_dpi', 100)))
        ...
        graph_dpi_entry.bind("<FocusOut>", lambda e: self.validate_and_mark('graph_dpi', self.graph_dpi_var.get(), int))
          # Export format
        ttk.Label(frame, text=TRANSLATIONS[self.language]['export_format']).grid(
            row=1, column=0, sticky=tk.W, padx=10, pady=5)
        
        self.export_format_var = tk.StringVar(value=user_preferences.get_preference('export_format', 'png'))
        ...
        export_format_combo.bind("<<ComboboxSelected>>", lambda e: self.mark_setting_changed('export_format', self.export_format_var.get()))
        ...
        self.export_dir_var = tk.StringVar(value=user_preferences.get_preference('last_export_directory', ''))
        ...
        export_dir_entry.bind("<FocusOut>", lambda e: self.mark_setting_changed('last_export_directory', self.export_dir_var.get()))
          # Browse button
        ...
        try:
            ...
            if value_type == int:
                ...
                if key == 'font_size' and (typed_value < 8 or typed_value > 72):
                    raise ValueError(f"Font size must be between 8 and 72")
                elif key == 'decimal_places' and (typed_value < 0 or typed_value > 15):
                    raise ValueError(f"Decimal places must be between 0 and 15")
                elif key == 'graph_dpi' and (typed_value < 50 or typed_value > 300):
                    raise ValueError(f"Graph DPI must be between 50 and 300")
                elif key == 'backup_interval_minutes' and (typed_value < 1 or typed_value > 1440):
                    raise ValueError(f"Backup interval must be between 1 and 1440 minutes")
            else:
            ...
            if key == 'font_size':
                self.font_size_var.set(str(user_preferences.get_preference('font_size', 12)))
            elif key == 'decimal_places':
                self.decimal_places_var.set(str(user_preferences.get_preference('decimal_places', 4)))
            elif key == 'graph_dpi':
                self.graph_dpi_var.set(str(user_preferences.get_preference('graph_dpi', 100)))
            elif key == 'backup_interval_minutes':
                self.backup_interval_var.set(str(user_preferences.get_preference('backup_interval_minutes', 30)))
    ...
    def save_settings(self):
        ...
        try:            # First validate all numeric settings
            if 'font_size' in self.modified_settings:
                self.validate_and_mark('font_size', str(self.modified_settings['font_size']), int)
            if 'decimal_places' in self.modified_settings:
                self.validate_and_mark('decimal_places', str(self.modified_settings['decimal_places']), int)
            ...
            if 'backup_interval_minutes' in self.modified_settings:
                self.validate_and_mark('backup_interval_minutes', str(self.modified_settings['backup_interval_minutes']), int)
